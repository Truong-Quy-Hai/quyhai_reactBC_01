...
export default class BaiTapRenderMap extends Component {
  ...
  renderFilm() {
    return this.dataFilm.map((film, i) => {
      return (
        <div className="col-4 mt-2">
          <div className="card">
            <div style={{background: `url(${film.hinhAnh}) lightblue`, backgroundRepeat: "no-repeat", backgroundSize: "cover", backgroundPosition: "center", width: "100%", height: "180px"}}>
              <img className="card-img-top" src={film.hinhAnh} style={{ opacity: 0,width: "100%", height: "100%", objectFit: "cover" }} alt="Card image cap" />
            </div>
            <div className="card-body">
              <h5 className="card-title">{film.tenPhim.length > 12 ? film.tenPhim.substr(0, 12) + "..." : film.tenPhim}</h5>
              <p style={{ height: "80px" }} className="card-text">{film.moTa.length > 100 ? film.moTa.substr(0, 100) + "..." : film.moTa}</p>
            </div>
          </div>
        </div>
      )
